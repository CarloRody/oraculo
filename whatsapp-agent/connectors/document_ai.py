...
DEFAULT_CHRONOLOGICAL_PROMPT_HEADER = (
    ...
    "O rótulo [recebido em DD/MM/AAAA] de cada resumo é APENAS a data em "
    "que o documento chegou pelo WhatsApp — em lotes de envio, vários "
    "documentos diferentes podem ter sido recebidos no mesmo dia. Essa "
    "lista NÃO está em ordem cronológica real ainda e o rótulo de "
    "recebimento NÃO deve ser usado pra ordenar.\n\n"
    "Sua tarefa é descobrir a data real de cada exame/laudo e então "
    "ordenar. Para cada resumo: leia o próprio texto e procure a data em "
    "que aquele exame/laudo específico foi realizado ou emitido (frases "
    "como \"realizado em\", \"emitido em\", \"data de entrada em\", "
    "\"laudo de\"). Cuidado com duas armadilhas comuns: (1) NUNCA use a "
    "data de nascimento do paciente como se fosse a data do exame; (2) "
    "NUNCA use uma data citada só como referência de comparação com um "
    "exame ou estudo ANTERIOR (ex.: \"em relação ao estudo de "
    "XX/XX/XXXX\") como se fosse a data do resumo atual — essa data "
    "pertence a um exame diferente, mencionado apenas para contexto. Se, "
    "depois disso, nenhuma data própria ficar identificável no texto, "
    "mantenha esse resumo na posição em que foi apresentado abaixo (como "
    "aproximação pelo recebimento) e deixe isso explícito na entrada "
    "(ex.: \"data do exame não identificada no texto\").\n\n"
    "Alguns documentos podem ser páginas ou mensagens separadas do mesmo "
    "laudo/exame que ainda não foram unidas manualmente num documento só — "
    "quando duas entradas com a mesma data real claramente pertencem ao "
    "mesmo laudo/exame, junte-as numa única entrada da linha do tempo, "
    "preservando TODOS os achados de ambas (nunca descarte um achado que "
    "apareça em só uma das duas).\n\n"
    "Sua tarefa: escrever UMA lista cronológica única, em português, "
    "ordenada pela data real de cada exame/laudo (mais antigo pro mais "
    "recente), cada exame/laudo aparecendo UMA ÚNICA VEZ. NÃO escreva uma "
    "segunda seção repetindo os mesmos exames como \"notas\" ou "
    "\"duplicados\", e NÃO escreva um resumo geral ou lista de tendências "
    "separada no final que re-liste as mesmas datas de novo — tendências, "
    "mudanças e alertas relevantes (comparando com o achado anterior, "
    "quando fizer sentido) devem aparecer DENTRO da própria entrada "
    "correspondente.\n\nResumos individuais, na ordem de recebimento:\n"
)
